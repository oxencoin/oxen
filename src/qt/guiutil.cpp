...
{
    // Convert oxen:// to oxen:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("oxen://"))
    {
        uri.replace(0, 12, "oxen:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Oxen.lnk";
}
...
{
    return GetAutostartDir() / "oxen.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Oxen-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  oxen-qt [" + tr("command-line options") + "]                     " + "\n";
...
{
    app.setStyleSheet("QWidget        { background: rgb(30,32,36); }"
                      "QFrame         { border: none; }"
                      "QComboBox      { color: rgb(255,255,255); }"
                      "QComboBox QAbstractItemView::item { color: rgb(255,255,255); }"
                      "QPushButton    { background: rgb(0,140,215); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(0,140,215); border-left-style: solid; border-left-width: 6px; }"
